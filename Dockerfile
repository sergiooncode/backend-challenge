...
RUN set -ex && \
  apt-get update --yes && \
  apt-get upgrade --yes && \
  apt-get install --no-install-recommends --yes curl build-essential libpq-dev gettext dnsutils procps strace libc-bin libexpat1 libcurl4-openssl-dev git libpcre3 libpcre3-dev libssl-dev postgresql && \
